...
TMUX= LAZYTMUX_NO_WATCH=1 PATH="$missing_root/bin:/usr/bin:/bin" TMUX_TMPDIR=/private/tmp HOME="$missing_root/home" \
  "$missing_root/bin/tmux" -L "$missing_server" -f /dev/null new-session -d -s "$missing_session"
TMUX= PATH="$missing_root/bin:/usr/bin:/bin" TMUX_TMPDIR=/private/tmp HOME="$missing_root/home" \
  "$missing_root/bin/tmux" -L "$missing_server" source-file "$REPO_ROOT/lazytmux.tmux"
messages=$(TMUX= PATH="$missing_root/bin:/usr/bin:/bin" TMUX_TMPDIR=/private/tmp HOME="$missing_root/home" "$missing_root/bin/tmux" -L "$missing_server" show-messages)
...
TMUX= PATH="$missing_root/bin:/usr/bin:/bin" TMUX_TMPDIR=/private/tmp HOME="$missing_root/home" "$missing_root/bin/tmux" -L "$missing_server" kill-server

starter_server="lazytmux-starter-missing-$$"
TMUX= LAZYTMUX_NO_WATCH=1 PATH="$missing_root/bin:/usr/bin:/bin" TMUX_TMPDIR=/private/tmp HOME="$missing_root/starter-home" \
  "$missing_root/bin/tmux" -L "$starter_server" -f /dev/null new-session -d -s "$missing_session-starter"
TMUX= PATH="$missing_root/bin:/usr/bin:/bin" TMUX_TMPDIR=/private/tmp HOME="$missing_root/starter-home" \
  "$missing_root/bin/tmux" -L "$starter_server" source-file "$REPO_ROOT/starter/.tmux.conf"
starter_messages=$(TMUX= PATH="$missing_root/bin:/usr/bin:/bin" TMUX_TMPDIR=/private/tmp HOME="$missing_root/starter-home" "$missing_root/bin/tmux" -L "$starter_server" show-messages)
...
TMUX= PATH="$missing_root/bin:/usr/bin:/bin" TMUX_TMPDIR=/private/tmp HOME="$missing_root/starter-home" "$missing_root/bin/tmux" -L "$starter_server" kill-server
